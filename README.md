...
1. **Success:** Fetches from API, updates the single DB record, returns fresh data.
2. **Network Failure/Timeout:** Retries 2 times. If total time hits 4.8s, aborts and returns the last successfully saved record from SQL Server.
3. **First Run Failure:** If the database is empty and the API is unreachable, returns `null` (HTTP 200 OK).
4. **Database Failure during save:** Returns the fresh data to the client without crashing, logs the DB error.
